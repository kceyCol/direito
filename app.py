...
if not os.path.exists(app.config['PROMPTS_FOLDER']):
    ...
    for categoria in ['geral', 'direito', 'licitacoes']:
...
PROMPTS_TIPOS = {
    ...
    'direito': ['fichamento_jurisprudencia', 'analise_critica_decisao', 'mapeamento_estrategia_argumentativa', 
                'analise_contrato_risco', 'explicacao_cliente_simplificado', 'gerador_clausulas_contratuais',
                'esquematizador_peca_processual', 'analise_impacto_legislativo', 'preparacao_inquiricao_testemunha',
                'sintese_entendimento_jurisprudencial'],
...
def gerar_prompt():
    dados = request.json
    categoria = dados.get('categoria', 'direito')
    tipo = dados.get('tipo', 'fichamento_jurisprudencia')
